...
def droom1():
    ...
    if not key and next_room is not None:
        if next_room != 3 or key:
...
def droom2():
    ...
    if not spider and next_room is not None:
        if next_room != 3 or key:
...
def droom9():
    ...
    if not tea and next_room is not None:
        if next_room != 3 or tea:
...
while not done:
    ...
    if v.upper() == "N":
        ...
        if next_room == 3 and not key:
            print("\nThe bedroom is locked, try to find the key\n")
        elif next_room == 3 and key and not unlock:
